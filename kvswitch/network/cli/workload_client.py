...
async def _send_one(
    ...
    # Include kvswitch_header if present (for KVSwitch baseline).
    if "kvswitch_header" in request:
        payload["kvswitch_header"] = request["kvswitch_header"]
    # Include prefix_hashes if present (for cache event tracking).
    if "prefix_hashes" in request:
    ...
    return {
        ...
        **{
        ...
        # L7 proxy routing annotation (present only for L7 baseline).
...
async def run_workload(
    ...
    for req in requests:
        ...
        tasks.append(asyncio.create_task(_send_one(req, host, port, timeout, t0)))
